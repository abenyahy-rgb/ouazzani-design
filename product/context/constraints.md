...
| C-06 | **La méthode est épinglée par le gitlink — et c'est LA CI qui le déplace.** `.gitmodules` ne porte que `path` et `url` : il n'épingle rien. Le gitlink épingle, et le semis le **réécrit**. L'épinglage n'est donc pas une garantie de stabilité pour le run : il garantit qu'à tout instant le registre exécuté est identifiable, pas qu'il ne changera pas. | Gitlink de l'arbre, **observé changer le 2026-09-21** : `a7b185b` → `359d6ce`, par le commit `09cbb66` de l'identité `launchpad` | K6 « Restriction d'outillage », qui a **détecté l'écart au moment où il s'est produit** : « le plugin exécuté est sur a7b185b, le dépôt produit épingle 359d6ce — la gouvernance tourne sur un autre registre que celui déclaré » |
...
**Observation du 2026-09-21, portée ici parce qu'elle corrige la révision 3 de ce fichier.**
C-06 énonçait, en révision 3, que la méthode est « épinglée, pas suivie au fil de l'eau ». Le
semis de la CI a déplacé le gitlink pendant la rédaction de l'activité 3, et le registre
exécuté a changé de version sans décision humaine. Trois conséquences constatées, non déduites :

1. **K6 a vu l'écart immédiatement** — le contrôle compare le commit exécuté au commit épinglé.
   La garde est préventive sur les chemins, K6 est détectif sur la provenance : c'est le second
   qui a mordu.
2. **Le registre a changé d'exigences.** La nouvelle version réclame une clé `map` dans
   `workspace-map.yaml` que la précédente n'exigeait pas : K7 est passé en `FAIL` sur un livrable
   déclaré `complet` et inchangé. **Un livrable conforme peut cesser de l'être sans être
   touché.**
3. **Aucune écriture n'a été refusée.** Le semis écrit hors `product/`, et la garde ne juge que
   `product/` (C-02). Les deux contraintes se composent exactement comme elles l'annonçaient.
